...
public class FT_4222_Spi_Master implements SpiMaster {
    ...
    public int singleReadWrite(byte[] readBuffer, byte[] writeBuffer, int sizeToTransfer, int[] sizeTransferred, boolean isEndTransaction) {
        ...
        sizeTransferred[0] = m98a(this.f75b, writeBuffer, readBuffer, sizeToTransfer);
        if (!isEndTransaction) {
            return 0;
        }
    ...
    public int multiReadWrite(byte[] readBuffer, byte[] writeBuffer, int singleWriteBytes, int multiWriteBytes, int multiReadBytes, int[] sizeOfRead) {
        ...
        bArr[2] = (byte) (multiWriteBytes & Telemetry.cbTagMax);
        bArr[3] = (byte) ((multiReadBytes & 65280) >> 8);
        bArr[4] = (byte) (multiReadBytes & Telemetry.cbTagMax);
        for (int i = 0; i < singleWriteBytes + multiWriteBytes; i++) {
            bArr[i + 5] = writeBuffer[i];
        }
        sizeOfRead[0] = m97a(this.f75b, bArr, readBuffer);
        return 0;
    }
    ...
    private int m97a(FT_Device fT_Device, byte[] bArr, byte[] bArr2) {
    ...
    private int m98a(FT_Device fT_Device, byte[] bArr, byte[] bArr2, int i) {
        int i2 = 0;
        ...
        while (i3 < length) {
            ...
            if (m99b(fT_Device, bArr3, bArr4) <= 0) {
                return -1;
            }
            for (byte b : bArr4) {
                bArr2[i5] = b;
        ...
        if (m99b(fT_Device, bArr5, bArr3) <= 0) {
            return -1;
        }
    ...
    private int m99b(FT_Device fT_Device, byte[] bArr, byte[] bArr2) {
        if (fT_Device == null || !fT_Device.isOpen()) {
            return -1;
        }
